...
def focus_stack(folder):
    '''
    performs focus stacking using images in folder
    :param folder: absolute path to the folder containing images to synthesize images
    :return: stacked images in numpy
    '''
    ext = set(["bmp", "jpeg", "jpg", "png", "tif", "tiff"])

    files = [x for x in os.listdir(folder) if x.split(".")[-1].lower() in ext]
    stacked = None
    for i in range(len(files) - 1):
        if i == 0:
            stacked = cv2.imread(os.path.join(folder, files[0]))
        image2 = cv2.imread(os.path.join(folder, files[i + 1]))
        stacked = parse_pairwise(stacked, image2)
    return stacked

def parse_pairwise(image1, image2, pyramid_min_size = 32):
    stacked_image = get_pyramid_fusion(np.array([image1,image2]), pyramid_min_size)
